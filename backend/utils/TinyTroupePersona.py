...
from backend.utils.TinyTroupePersonaAction import TinyTroupePersonaAnalysis, TinyTroupePersonaQNA
...
@dataclass
class TinyTroupePersona(Persona):
    prompt: Optional[str] = None
    gpt_prompt: Optional[str] = None
    population_size: Optional[int] = default_population_size
    agents: Optional[List[Agent]] = None
    analysis: Optional[TinyTroupePersonaAnalysis] = None
    qna: Optional[TinyTroupePersonaQNA] = None
    ...
    def __init__(self, name: str, description: str, 
                 ...
                 analysis: Optional[TinyTroupePersonaAnalysis] = None, qna: Optional[TinyTroupePersonaQNA] = None,
                 ppt: Optional[PPT] = None) -> None:
        super().__init__(name=name, description=description, id=id, type=PersonaType.TINY_TROUPE)
        self.prompt = prompt
        self.gpt_prompt = gpt_prompt
        self.population_size = population_size
        self.agents = agents or []
        self.analysis = analysis or TinyTroupePersonaAnalysis()
        self.qna = qna or TinyTroupePersonaQNA()
        self.ppt = ppt
        self.gpt = GPT(f"GPT Assistant {self.name}", Constants.Prompts.TINY_TROUPE_INFO)
        self._set_mapping()

    @classmethod
    def load(cls, persona_dict: Dict[str, Any], root_folder: str = None) -> 'TinyTroupePersona':
        return cls(
            ...
            analysis=TinyTroupePersonaAnalysis.load(persona_dict.get('analysis', {})),
            qna=TinyTroupePersonaQNA.load(persona_dict.get('qna', {}))
        )
    ...
    def create_tiny_world(self) -> TinyWorld:
        return TinyWorld(f"TinyWorld {self.name}", 
    ...
    def run(self, use_existing_agents:bool=True):
        self.id = str(uuid4())
        logger.info(f"Running TinyTroupe Persona {self.name}...")
        ...
        tinyWorld = self.create_tiny_world()

        # Analyze and summarize
        logger.debug("Analyzing and summarizing...")
        self.update_mapping() # Update mapping of variables to substitute in prompts
        self.analysis.run(tinyWorld = tinyWorld, updateDict=self.mappingDict)
        
        # Ask questions and summarize
        logger.debug("Asking questions and summarizing...")
        self.update_mapping() # Update mapping of variables to substitute in prompts
        self.qna.run(tinyWorld = tinyWorld, updateDict=self.mappingDict)
